...
// Stability describes how often a message's content is expected to change
// between consecutive LLM requests. It is set where the prompt is assembled and
// lets a provider decide where a cached prefix can safely end; providers
// without client-side cache control ignore it.
type Stability int

const (
	// StabilityVolatile content changes turn to turn: live conversation
	// messages, retrieved memories, error context. This is the zero value so
	// messages that never set Stability behave as before.
	StabilityVolatile Stability = iota

	// StabilitySession content is stable for the life of a session unless
	// something structural changes: the tool listing, compacted history blocks.
	StabilitySession

	// StabilityStatic content is stable for the life of the binary: capability
	// text, custom instructions, repository context.
	StabilityStatic
)
...
type Message struct {
	...
	// Stability classifies how often Content changes between requests.
	Stability Stability
}
...
// WithStability sets the message's stability and returns the message for chaining.
func (m *Message) WithStability(stability Stability) *Message {
